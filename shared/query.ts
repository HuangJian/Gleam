 ...
 *
 * This module is pure (no infra/IO deps) so it is trivially unit-testable and
 * shared between client and backend. It is the product's "Recall" capability.
 *
...
// ---------------------------------------------------------------------------
// URL helper (inlined here so this module has zero internal deps)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Keyword extraction (for highlight generation)
// ---------------------------------------------------------------------------

/**
 * Extracts bare keyword values from a query AST. These are the free-text terms
 * that should be highlighted in search results. Filter values (tag:, domain:,
 * date operators, periods) are excluded — they match structurally, not as
 * substrings of the gleam text.
 *
 * Example:
 *   "react hooks #frontend >=20260101"
 *   → ["react", "hooks"]
 *
 *   "(machine learning OR deep learning) tag:ai"
 *   → ["machine learning", "deep learning"]
 */
export function extractKeywords(ast: QueryNode | null): string[] {
  if (!ast) return []
  ...
  function walk(node: QueryNode): void {
    switch (node.kind) {
      case 'keyword':
        if (node.value) keywords.push(node.value)
        break
      case 'not':
        // Don't extract keywords from negated subtrees — highlighting a term
        // that must NOT appear would be confusing.
        break
      case 'and':
      case 'or':
        walk(node.left)
        walk(node.right)
        break
      // term, datecmp, period — not keywords, skip
    }
  }
